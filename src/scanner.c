...
int scan_destinations(Scanner_t *scanner, Destination_addresses_t *destination, Source_address_t *source, Table_packet_t *table){
    ...
    if(err != EXIT_OK) RETURN_ERROR(ERR_SOCK_INIT, "Failed to initialize sockets");
    ...
    // Jednovláknová smyčka: přijímej, kontroluj timeouty, případně resendi
    int pending = 1;
    while (pending) {
        ...
        if (p_fds[i].revents & POLLIN) {
            ...
            while (1) {//prisk:: zavisi na MSG_CONTWAIT
                ...
                if (i == 0 || i == 1) { 
                    ...
                    struct tcphdr *tcph;
                    if (i == 0) { // IPv4
                        if (len < (int)(sizeof(struct iphdr) + sizeof(struct tcphdr))) continue;
                        struct iphdr *iph = (struct iphdr *)buffer;
                        tcph = (struct tcphdr *)(buffer + (iph->ihl * 4));
                    } else { // IPv6
                        if (len < (int)sizeof(struct tcphdr)) continue;
                        tcph = (struct tcphdr *)buffer;
                    }
                    res_port = ntohs(tcph->source);
                    res_ack = ntohl(tcph->ack_seq);
                    if (tcph->syn && tcph->ack) res_status = ST_OPEN;
                    else if (tcph->rst) res_status = ST_CLOSED;
                } 
                // --- ANALÝZA (ICMP / UDP) ---
                else {
                    res_proto = SCAN_UDP;
                    int offset = (i == 2) ? (20 + 8 + 20) : (8 + 40); 
                    if (len < offset + (int)sizeof(struct udphdr)) continue;

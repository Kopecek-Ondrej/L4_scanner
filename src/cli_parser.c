...
int eval_arguments(Arguments_t* args, Cli_Parser_t* parser) {
    ...
    if(args->t_ports != NULL)
        parser->tcp_use = true;
    if(args->u_ports != NULL)
        parser->udp_use = true;
...
int eval_ports(char* s_ports, Ports_t* ports) {
    ...
    if(strchr(s_ports, '-') != NULL) {
        ...
        ports->type = MULTIP;
    } else if(strchr(s_ports, '.')) { // simple fix
        RETURN_ERROR(ERR_CLI_ARG, "Invalid decimal point in list");
    } else { // single number
        if(contains_letter(s_ports)) {
            RETURN_ERROR(ERR_CLI_ARG, "Invalid letters in port definition");
        }
        char* end;
